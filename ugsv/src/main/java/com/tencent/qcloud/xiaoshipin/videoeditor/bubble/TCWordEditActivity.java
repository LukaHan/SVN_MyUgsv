...
public class TCWordEditActivity extends FragmentActivity implements View.OnClickListener,
        ...
        TCWordInputDialog.OnWordInputCallback, TCBubbleSettingView.OnWordInfoCallback,
        TCWordBubbleView.IOperationViewClickListener, TCLayerViewGroup.OnItemClickListener {
    ...
    private LinearLayout mLayoutWordList;
    //==================================播放布局==================================
    private ImageView mIvPlay;
    private TextView mTvAdd; //添加字幕按钮
    ...
    private void initViews() {
        ...
        mLayoutWordList = (LinearLayout) findViewById(R.id.layout_word_list);
    ...
    private void onClickPlay() {
        if (mCurrentState == STATE_PAUSE || mCurrentState == PlayState.STATE_PREVIEW_AT_TIME) {
            resumePlay();
    ...
    private void startPlay() {
        if (mCurrentState == STATE_NONE || mCurrentState == STATE_STOP) {
            ...
            mIvPlay.setImageResource(R.drawable.ic_pause_selector);
            // 后台切换回来的时候 需要隐藏掉这个
            mTCBubbleViewGroup.setVisibility(View.INVISIBLE);
        }
    }
    ...
    private void pausePlay(boolean isShow) {
        ...
        if (mCurrentState == STATE_START || mCurrentState == STATE_RESUME) {
            ...
            mIvPlay.setImageResource(R.drawable.ic_play_selector);
        } else if (mCurrentState == PlayState.STATE_PREVIEW_AT_TIME) {
            mCurrentState = STATE_PAUSE;
            mTXVideoEditer.pausePlay();
            mIvPlay.setImageResource(R.drawable.ic_play_selector);
